...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_npu', action='store_true', help='Enable NPU acceleration')  # 添加NPU参数
    ...
    # 根据设备类型设置混合精度上下文
    if device_type == "npu":
        ctx = nullcontext() if device_type == "cpu" else torch.npu.amp.autocast()
    else:
    ...
    # 根据设备类型设置梯度缩放器
    if device_type == "npu":
        scaler = torch.npu.amp.GradScaler(enabled=(args.dtype in ['float16', 'bfloat16']))
    else:
